...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runSarsa(BasicGridWorld gen, Domain domain,
							 State initialState, RewardFunction rf, TerminalFunction tf,
							 SimulatedEnvironment env, boolean showPolicyMap) {
		System.out.println("//SARSA Analysis//");
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			agent.initializeForPlanning(rf, tf, 1);
			p = agent.planFromState(initialState);
			AnalysisAggregator.addSarsaReward(calcRewardInEpisode(ea));
			AnalysisAggregator.addMillisecondsToFinishSarsa((int) (System.nanoTime()-startTime)/1000000);
			AnalysisAggregator.addStepsToFinishSarsa(ea.numTimeSteps());

		}
		AnalysisAggregator.printSarsaResults();
		MapPrinter.printPolicyMap(getAllStates(domain,rf,tf,initialState), p, gen.getMap());
		System.out.println("\n\n");

		//visualize the value function and policy.
		if(showPolicyMap){
			simpleValueFunctionVis((ValueFunction)agent, p, initialState, domain, hashingFactory, "SARSA");
		}
